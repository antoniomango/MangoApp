...
// ═══════════════════════════════════════════════════════════════════════════
// RAGGRUPPAMENTO ORDINI PER SETTIMANA DI SCADENZA — componente condiviso
// (operatore.html + responsabile.html, ovunque si mostri la lista ordini)
// ═══════════════════════════════════════════════════════════════════════════
// Serve a trovare più in fretta gli ordini cartacei in produzione: la lista viene
// spezzata in "Senza scadenza" → "Scadute" → un gruppo per ogni settimana ISO 8601
// (lunedì-domenica, dalla settimana corrente in poi). Nessun dato salvato nel DB: la
// settimana si calcola al volo dal campo `ordini.scadenza`.

// Parsing 'YYYY-MM-DD' come data UTC pura — evita lo scivolamento di un giorno che
// `new Date('2026-09-10')` interpretato in fuso locale può causare vicino a mezzanotte.
function _mangoParseYMD(s) {
  const [y, m, d] = String(s).split('-').map(Number);
  return new Date(Date.UTC(y, m - 1, d));
}

// Numero di settimana ISO 8601: settimana che inizia di lunedì, la prima dell'anno è
// quella che contiene il primo giovedì. Ritorna { isoYear, week } — isoYear può
// differire dall'anno civile a cavallo di dicembre/gennaio.
function mangoSettimanaIso(dataUTC) {
  const d = new Date(Date.UTC(dataUTC.getUTCFullYear(), dataUTC.getUTCMonth(), dataUTC.getUTCDate()));
  const giornoLun0 = (d.getUTCDay() + 6) % 7;       // lun=0 … dom=6
  d.setUTCDate(d.getUTCDate() - giornoLun0 + 3);     // porta al giovedì della stessa settimana
  const isoYear = d.getUTCFullYear();
  const primoGiovedi = new Date(Date.UTC(isoYear, 0, 4));
  const primoGiovediLun0 = (primoGiovedi.getUTCDay() + 6) % 7;
  primoGiovedi.setUTCDate(primoGiovedi.getUTCDate() - primoGiovediLun0 + 3);
  const week = 1 + Math.round((d - primoGiovedi) / (7 * 24 * 3600 * 1000));
  return { isoYear, week };
}

// Lunedì e domenica (Date UTC) di una data settimana ISO.
function mangoRangeSettimanaIso(isoYear, week) {
  const gen4 = new Date(Date.UTC(isoYear, 0, 4));
  const gen4Lun0 = (gen4.getUTCDay() + 6) % 7;
  const lunediSett1 = new Date(gen4);
  lunediSett1.setUTCDate(gen4.getUTCDate() - gen4Lun0);
  const lunedi = new Date(lunediSett1);
  lunedi.setUTCDate(lunediSett1.getUTCDate() + (week - 1) * 7);
  const domenica = new Date(lunedi);
  domenica.setUTCDate(lunedi.getUTCDate() + 6);
  return { lunedi, domenica };
}

function _mangoGGMM(d) {
  return String(d.getUTCDate()).padStart(2, '0') + '/' + String(d.getUTCMonth() + 1).padStart(2, '0');
}

// Chiave ordinabile lessicograficamente = cronologicamente (isoYear-Wnn).
function _mangoChiaveSettimana(isoYear, week) {
  return isoYear + '-W' + String(week).padStart(2, '0');
}

// Raggruppa e ordina una lista di ordini per settimana di scadenza.
// `ordini`: oggetti con almeno { scadenza: 'YYYY-MM-DD'|null, stato, creato_il }.
// Ritorna un array ORDINATO di gruppi:
//   { tipo: 'senza'|'scadute'|'settimana', chiave, label, isSettimanaCorrente, ordini: [...] }
// Gli ordini dentro ogni gruppo sono già ordinati: scadenza crescente; per 'senza',
// creato_il crescente. La funzione NON filtra e NON modifica gli ordini: raggruppa
// soltanto — chi chiama passa già la lista filtrata come serve (ricerca, filtri di
// stato, ecc.), così i filtri/ricerche esistenti restano invariati.
function raggruppaOrdiniPerScadenza(ordini) {
  const now = new Date();
  const oggiUTC = new Date(Date.UTC(now.getFullYear(), now.getMonth(), now.getDate()));
  const oggiStr = oggiUTC.toISOString().slice(0, 10);
  const scOggi = mangoSettimanaIso(oggiUTC);
  const chiaveSettCorrente = _mangoChiaveSettimana(scOggi.isoYear, scOggi.week);

  const senza = [];
  const scadute = [];
  const perSettimana = new Map(); // chiave → { isoYear, week, ordini: [] }

  for (const o of (ordini || [])) {
    if (!o.scadenza) { senza.push(o); continue; }
    // "Scadute": scadenza nel passato E non già spedito/archiviato. Le liste che usano
    // oggi questa funzione contengono solo ordini non spediti, ma il controllo resta
    // esplicito nel caso la funzione venga riusata su una lista diversa.
    const spedito = o.stato === 'spedito' || o.stato === 'archiviato';
    if (o.scadenza < oggiStr && !spedito) { scadute.push(o); continue; }
    const w = mangoSettimanaIso(_mangoParseYMD(o.scadenza));
    const chiave = _mangoChiaveSettimana(w.isoYear, w.week);
    if (!perSettimana.has(chiave)) perSettimana.set(chiave, { isoYear: w.isoYear, week: w.week, ordini: [] });
    perSettimana.get(chiave).ordini.push(o);
  }

  const perScadenzaAsc = (a, b) => String(a.scadenza || '').localeCompare(String(b.scadenza || ''));
  const perCreazioneAsc = (a, b) => new Date(a.creato_il || 0) - new Date(b.creato_il || 0);

  const gruppi = [];
  if (senza.length) {
    gruppi.push({ tipo: 'senza', chiave: '0-senza', label: 'Senza scadenza',
      isSettimanaCorrente: false, ordini: senza.slice().sort(perCreazioneAsc) });
  }
  if (scadute.length) {
    gruppi.push({ tipo: 'scadute', chiave: '1-scadute', label: 'Scadute',
      isSettimanaCorrente: false, ordini: scadute.slice().sort(perScadenzaAsc) });
  }
  Array.from(perSettimana.values())
    .sort((a, b) => _mangoChiaveSettimana(a.isoYear, a.week).localeCompare(_mangoChiaveSettimana(b.isoYear, b.week)))
    .forEach(g => {
      const r = mangoRangeSettimanaIso(g.isoYear, g.week);
      gruppi.push({
        tipo: 'settimana',
        chiave: '2-' + _mangoChiaveSettimana(g.isoYear, g.week),
        label: `Settimana ${g.week} — dal ${_mangoGGMM(r.lunedi)} al ${_mangoGGMM(r.domenica)}`,
        isSettimanaCorrente: _mangoChiaveSettimana(g.isoYear, g.week) === chiaveSettCorrente,
        ordini: g.ordini.slice().sort(perScadenzaAsc)
      });
    });
  return gruppi;
}
